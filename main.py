...
class TrackOptimizerApp:
        ...
        if mode == "edit":
            if clicked_idx is not None:
                ...
                if self.edit_submode == "move":
                    ...
                    self.manual_angles[self.selected_point] = True
            ...
            if clicked_idx is None:
                ...
                self.manual_angles.append(False)
                self.recalculate_auto_angles()
                self.update_canvas_drawings()
    ...
    def calculate_lap_telemetry(self, smooth_line):
        ...
        MU = 3.5; G = 9.81; MAX_ACCEL = 320.0; MAX_BRAKE = 480.0
        MAX_SPEED = 520.0; MIN_SPEED = 55.0; PIXEL_SCALE = 0.35
        ...
        for i in range(n):
            p1 = smooth_line[i - 1]; p2 = smooth_line[i]; p3 = smooth_line[(i + 1) % n]
            a = p2 - p1; b = p3 - p1
...
# ── DEPLOYMENT-WRAPPER FÜR DIE WEB-DARSTELLUNG ──────────────────────────────
if __name__ == "__main__":
    ...
    # Öffnet deine unveränderte App direkt in einer modernen Webview-Umgebung
    webview.create_window(
        "F1 Track Optimizer – Web App", 
        start_app, 
        width=1250, 
        height=750
    )
